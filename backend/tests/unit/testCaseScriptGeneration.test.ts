...
test("resolveScriptGenerationOnSave resets run state only when generation is newly required", () => {
  assert.deepEqual(
    resolveScriptGenerationOnSave(
      {
    ...
    { scriptNeedsGeneration: true, resetRunState: true, clearScript: true },
  );
  ...
  assert.deepEqual(
    ...
    { scriptNeedsGeneration: false, resetRunState: false, clearScript: false },
  );
  ...
  assert.deepEqual(
    ...
    { scriptNeedsGeneration: true, resetRunState: true, clearScript: false },
  );
});
