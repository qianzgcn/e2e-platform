...
6. 生成脚本后必须运行一次对应 spec：`npm run test:generated -- tests/generated/{id}.spec.ts`。
7. 如果执行失败且是脚本问题（locator 不准、等待不足、作用域错误、数据幂等不足、断言写法不稳），读取 Playwright 输出和 error-context，重新探测页面并修复脚本，然后再运行一次。
8. 如果确认不是脚本可修复问题（页面/环境不可达、账号无权限、自然语言步骤与页面不一致、测试数据不存在等），不要编造通过脚本；仍要覆盖目标 spec，内容写成一个直接 `throw new Error("用例不可执行：具体原因")` 的失败用例，让平台能记录该条用例的具体失败原因。
9. 直接创建或覆盖目标文件，不要只在终端输出代码。
